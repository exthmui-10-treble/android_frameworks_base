...
public class StatusBarIconView extends AnimatedImageView implements StatusIconDisplayable {
    ...
    private boolean updateDrawable(boolean withClear) {
        ...
        if (mIconDrawable instanceof BitmapDrawable && ((BitmapDrawable) mIconDrawable).getBitmap() != null) {
            // If it's a bitmap we can check the size directly
            int byteCount = ((BitmapDrawable) mIconDrawable).getBitmap().getByteCount();
            if (byteCount > MAX_BITMAP_SIZE) {
                Log.w(TAG, "Drawable is too large (" + byteCount + " bytes) " + mIcon);
                return false;
            }
        } else if (mIconDrawable.getIntrinsicWidth() > MAX_IMAGE_SIZE
                || mIconDrawable.getIntrinsicHeight() > MAX_IMAGE_SIZE) {
            // Otherwise, check dimensions
            Log.w(TAG, "Drawable is too large (" + mIconDrawable.getIntrinsicWidth() + "x"
                    + mIconDrawable.getIntrinsicHeight() + ") " + mIcon);
            return false;
        }
